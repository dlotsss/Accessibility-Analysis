...
svm.model = svm(Wheelchair.Accessible ~ Vehicle.Type + Status + Vehicle.Model.Year 
                   + Public.Vehicle.Number + ZIP.Code + CompanyGroup
                   + RecordsGroup + DatesGroup, data = vehicles.train, cost = 10)

predict_svm <- predict(model_svm, vehicles.train)

table(predict_svm, vehicles.train$Wheelchair.Accessible)
mean(predict_svm == vehicles.train$Wheelchair.Accessible, na.rm=TRUE)
```
