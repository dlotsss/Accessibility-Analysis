...
svm.model = svm((Wheelchair.Accessible ~ Vehicle.Model.Year 
                   + Public.Vehicle.Number + ZIP.Code + CompanyGroup
                   + RecordsGroup, data = vehicles.train, cost = 10)

predict_svm <- predict(svm.model, vehicles.train)

```

```{r}
sqrt(mean((vehicles.train$Wheelchair.Accesible - predict_svm)^2))

table(predict_svm, vehicles.train$Wheelchair.Accesible)
mean(predict_svm == vehicles.train$Wheelchair.Accesible)
```


```{r}
predict_svm_test <- predict(svm.model, vehicles.test$WheelchairAccesible) 
sqrt(mean((vehicles.test$WheelchairAccesible - predict_svm_test)^2))
```
