...
KNN is not working because I have too many categorical values, therefore I need to use other methods. 
...
library(ggdendro)
library(ggplot2)
agn1 = agnes(vehicles., metric = "manhattan", diss=FALSE)

d <- dist(vehicles.train)
hcl <- hclust(d, method = "complete")
ck = cutree(hcl, k = 3)
ck = factor(ck)

hcd <- as.dendrogram(hcl)
plot(cut(hcd, h=3)$upper, main="Specific height", hang = -1)

ggdendrogram(hcl, rotate = FALSE) +
  theme(axis.line.y = element_line() )

pca = prcomp(vehicles.train)

veh <- as.data.frame(pca$x)   
veh$Species <- vehicles.train$Species   

ggplot(data = veh) + 
  geom_point(mapping = aes(x = PC1, y = PC2, color = ck, shape = vehicles.train$Species))
